...
MEM_TTL    = 1800        # seconds before cache entry expires (30 min for daily data)

# Per-symbol fundamentals quick cache (survives company switching)
_fund_cache = {}   # sym → {data, ts}
FUND_TTL    = 3600 # fundamentals cache: 1 hour

def fund_cache_get(sym):
    entry = _fund_cache.get(sym)
    if entry and (time.time() - entry["ts"]) < FUND_TTL:
        return entry["data"]
    return None

def fund_cache_set(sym, data):
    _fund_cache[sym] = {"data": data, "ts": time.time()}

def cache_set(key, data, ttl=None):
    """Store in memory cache. ttl overrides MEM_TTL (seconds)."""
    with _mem_lock:
        _mem_cache[key] = {"data": data, "ts": time.time(), "ttl": ttl or MEM_TTL}

def cache_get(key):  # redefine to use per-entry TTL
    with _mem_lock:
        entry = _mem_cache.get(key)
        if entry and (time.time() - entry["ts"]) < entry.get("ttl", MEM_TTL):
            return entry["data"]
    return None
...
# Run init_db in a background thread so port binds instantly on Render
# A _db_ready event signals completion; requests wait on it if needed
...
threading.Thread(target=_init_db_bg, daemon=True, name="init_db").start()

def wait_db():
    """Call at the start of any handler that needs the DB."""
...
def compute_indicators(sym):
    ...
    # Check fast fund cache first (survives company switches)
    cached = fund_cache_get(sym) or cache_get(cache_key)
    if cached:
        return cached
    ...
    cache_set(cache_key, result)
    fund_cache_set(sym, result)   # also save to fast fund cache
    return result
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_get(self):
        wait_db()   # ensure DB is initialised before any query
        ...
        # ── /intraday?sym=TCS&interval=5m ───────────────────────────────────
        # Returns intraday OHLCV (5m, 15m, 30m, 60m) for last 1–5 days.
        # yfinance rule: interval<1d only available for last 60 days max.
        if parsed.path == "/intraday":
            sym      = qs.get("sym",      [""])[0].upper().strip()
            interval = qs.get("interval", ["5m"])[0].strip()   # 5m|15m|30m|60m
            days_back= int(qs.get("days", ["1"])[0])           # 1 or 5
            if not sym:
                self.send_json({"ok": False, "error": "Missing sym"}, 400); return
            ...
            # Choose period string based on days_back
            if days_back <= 1:
                period = "1d"
            elif days_back <= 5:
                period = "5d"
            ...
            cache_key = f"intraday:{sym}:{interval}:{period}"
            cached = cache_get(cache_key)
            if cached:
                self.send_json(cached); return

            print(f"  [INTRADAY] {sym}.NS  interval={interval}  period={period}")
            df = yf_fetch(lambda: yf.Ticker(f"{sym}.NS").history(
            ...
            if df is None or df.empty:
                self.send_json({"ok": False, "error":
                    f"No intraday data for {sym}. Market may be closed or symbol invalid."}, 400)
                return
            ...
            for idx, r in df.iterrows():
                c = float(r.get("Close", 0) or 0)
                if c <= 0: continue
                # Format timestamp: include time for intraday
                try:
            ...
            if not rows:
                self.send_json({"ok": False, "error": "No valid intraday candles returned."}, 400)
                return
            ...
            # Cache intraday for 5 minutes only (it's live data)
            cache_set(cache_key, result, ttl=60)  # intraday: 60s cache only
    ...
    def _handle_post(self, path):
        wait_db()   # ensure DB is ready before auth queries
        # ── /auth/register ───────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    # ── CRITICAL: bind port IMMEDIATELY so Render detects it ─────────────────
    # Render gives ~60s to bind a port. We bind first, do all heavy init after.
    server = ThreadingHTTPServer(("0.0.0.0", PORT), Handler)
    print(f"[OK] Port {PORT} bound — server accepting connections")

    db_mb = round(DB_PATH.stat().st_size/1024/1024, 1) if DB_PATH.exists() else 0
    print("=" * 62)
    print("  NSE Stock Analyzer — Server v8")
    ...
    print(f"  CORS    : GET, POST, OPTIONS allowed from *")
    print("=" * 62)
    try:
