"""
NSE Stock Analyzer — Server v6
SQLite-backed, scheduled auto-updates, no timeout failures.

Database update schedule:
  • Price OHLCV     -> every 5 minutes  (live market data)
  • Volume summary  -> every 60 minutes
  • Fundamentals    -> once daily at 08:00 IST
  • History         -> 10-year rolling window, filled on first use per symbol

Endpoints (unchanged from v5):
  GET /ping
  GET /history?sym=TCS&start=2016-01-01&end=2026-04-16
  GET /forecast?sym=TCS&days=14&model=lstm|prophet|ensemble
  GET /indicators?sym=TCS
  GET /db_status
"""

from http.server import BaseHTTPRequestHandler, HTTPServer
from urllib.parse import urlparse, parse_qs
import json, traceback, warnings, datetime, os, math, sqlite3, threading, time, pathlib
...
# On Render: copy bundled DB to /tmp for write access (repo filesystem is read-only)
...
if _render:
    ...
    if DB_PATH.exists() and (not _tmp_db.exists() or _tmp_db.stat().st_size < DB_PATH.stat().st_size):
        print(f"[DB] Copying repo DB to /tmp ({round(DB_PATH.stat().st_size/1024/1024,1)} MB)...")
...
# ============================================================
# RETRY HELPER  -- no AbortSignal, no hard timeout
# ============================================================
def yf_fetch(func, retries=5, base_delay=4):
    """
    Retry func() up to `retries` times with exponential back-off.
    Never raises a network/timeout error -- returns None on total failure
    so callers can fall back to cached DB data.
    """
    for attempt in range(retries):
        try:
            ...
            wait = base_delay * (2 ** attempt)   # 4, 8, 16, 32, 64 s
            print(f"  [RETRY {attempt+1}/{retries}] {type(e).__name__}: {e} -- waiting {wait}s")
            time.sleep(wait)
    print(f"  [RETRY] All {retries} attempts failed -- returning None")
    return None

# ============================================================
# HISTORY SEEDER  -- 10 years
# ============================================================
_active_symbols = set()
...
def ensure_history(sym, conn=None):
    """
    Ensure 10yr daily OHLCV is in DB.
    CACHE LOGIC:
    - If we have >= 200 rows AND newest_date is within last 2 days -> already good, skip.
    - If newest_date is older -> only fetch MISSING days (incremental update).
    - If no rows at all -> full 10yr seed.
    This means once data is seeded, subsequent requests are instant (DB hit).
    """
    ...
    # Check both meta AND actual row count
    row = conn.execute(
    ...
    # Already well-populated and fresh within 2 days (covers weekends/holidays)
    ...
    if row_count == 0:
        # No data at all — full 10yr seed
        fetch_start = ten_yr_start
        print(f"  [DB] Full 10yr seed for {sym}...")
    else:
        # Incremental — only fetch days since last entry
        fetch_start = row[1] if row and row[1] else ten_yr_start
        print(f"  [DB] Incremental update for {sym} from {fetch_start}...")

    end = str(datetime.date.today() + datetime.timedelta(days=1))
    df  = yf_fetch(lambda: yf.Ticker(f"{sym}.NS").history(
        start=fetch_start, end=end, interval="1d", auto_adjust=True))

    if df is None or df.empty:
        print(f"  [DB] Could not fetch {sym} — will use existing cache or live fallback")
        if close_conn: conn.close()
        return
    ...
    with DB_LOCK:
        ...
        # Recompute oldest from actual DB (handles incremental correctly)
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (sym, oldest_in_db, newest, today))
        conn.commit()

    total = conn.execute("SELECT COUNT(*) FROM prices WHERE symbol=?", (sym,)).fetchone()[0]
    print(f"  [DB] {sym}: +{len(rows)} new rows, {total} total ({oldest_in_db} to {newest})")
    if close_conn: conn.close()
...
def sync_live_prices():
    with _active_lock:
        syms = list(_active_symbols)
    if not syms:
        return
    print(f"  [SYNC] Live prices for {len(syms)} symbols...")
    for sym in syms:
        try:
            df = yf_fetch(lambda s=sym: yf.Ticker(f"{s}.NS").history(period="2d", interval="1m", auto_adjust=True), retries=3, base_delay=3)
            if df is None or df.empty:
                continue
            conn  = get_db()
            rows  = [(sym, int(idx.timestamp()), round(float(r.get("Close",0) or 0),2), int(r.get("Volume",0) or 0))
                     for idx, r in df.iterrows() if float(r.get("Close",0) or 0) > 0]
            today = str(datetime.date.today())
            ddf   = yf_fetch(lambda s=sym: yf.Ticker(f"{s}.NS").history(period="2d", interval="1d", auto_adjust=True), retries=2, base_delay=2)
            with DB_LOCK:
                conn.executemany("INSERT OR REPLACE INTO prices_live(symbol,ts,price,volume) VALUES(?,?,?,?)", rows)
                if ddf is not None and not ddf.empty:
                    lr = ddf.iloc[-1]
                    conn.execute("INSERT OR REPLACE INTO prices(symbol,date,open,high,low,close,volume) VALUES(?,?,?,?,?,?,?)",
                        (sym, today,
                         round(float(lr.get("Open",  0) or 0),2), round(float(lr.get("High",  0) or 0),2),
                         round(float(lr.get("Low",   0) or 0),2), round(float(lr.get("Close", 0) or 0),2),
                         int(lr.get("Volume",0) or 0)))
                    conn.execute("""INSERT INTO history_meta(symbol,oldest_date,newest_date,last_price_sync)
                        VALUES(?,COALESCE((SELECT oldest_date FROM history_meta WHERE symbol=?),'2016-01-01'),?,?)
                        ON CONFLICT(symbol) DO UPDATE SET newest_date=excluded.newest_date,last_price_sync=excluded.last_price_sync
                    """, (sym, sym, today, today))
                conn.commit()
            conn.close()
        except Exception as e:
...
def sync_hourly_volume():
    with _active_lock:
        syms = list(_active_symbols)
    if not syms:
        return
    print(f"  [SYNC] Hourly volume for {len(syms)} symbols...")
    for sym in syms:
        try:
            df = yf_fetch(lambda s=sym: yf.Ticker(f"{s}.NS").history(period="1d", interval="60m", auto_adjust=True), retries=3, base_delay=3)
            if df is None or df.empty: continue
            conn = get_db()
            rows = [(sym, idx.strftime("%Y-%m-%d %H:00"), int(r.get("Volume",0) or 0)) for idx, r in df.iterrows()]
            with DB_LOCK:
                conn.executemany("INSERT OR REPLACE INTO volume_hourly(symbol,hour,volume) VALUES(?,?,?)", rows)
                conn.commit()
            conn.close()
        except Exception as e:
            print(f"  [SYNC-VOL] {sym}: {e}")
...
def sync_fundamentals(sym, force=False):
    ...
    if row and row[0] == today and not force:
        conn.close()
        return
    print(f"  [DB] Syncing fundamentals for {sym}...")
    try:
        info = yf_fetch(lambda s=sym: yf.Ticker(f"{s}.NS").info or {}, retries=4, base_delay=5) or {}
        def safe(k, d=None):
            v = info.get(k)
            return v if (v is not None and v != "N/A" and v != 0) else d
        rev_growth = None
        try:
            fin = yf_fetch(lambda s=sym: yf.Ticker(f"{s}.NS").financials, retries=3, base_delay=4)
        ...
        with DB_LOCK:
            ...
            conn.execute("""INSERT INTO history_meta(symbol,oldest_date,newest_date,last_fund_sync)
            ...
            conn.commit()
        conn.close()
        print(f"  [DB] Fundamentals cached for {sym}")
    except Exception as e:
        print(f"  [DB] Fund error {sym}: {e}")
        try: conn.close()
        except: pass
...
def _scheduler():
    ...
    while True:
        ...
        now_ts = time.time()
        if now_ts - last_vol_sync >= 3600:
            try: sync_hourly_volume()
            except Exception as e: print(f"  [SCHED] vol: {e}")
            last_vol_sync = now_ts
...
print("[OK] Scheduler started (5-min prices, hourly volume, daily fundamentals at 08:00 IST)")
...
def get_stock_data(sym, start, end):
    register_symbol(sym)
    conn = get_db()
    ensure_history(sym, conn)
    rows_db = conn.execute(
        "SELECT date,open,high,low,close,volume FROM prices WHERE symbol=? AND date>=? AND date<=? ORDER BY date ASC",
        (sym, start, end)
    ).fetchall()
    fund = conn.execute(
        "SELECT mkt_price,day_open,day_high,day_low,avg_volume,high52,low52,prev_close FROM fundamentals WHERE symbol=?", (sym,)
    ).fetchone()
    conn.close()

    if not rows_db:
        return _live_fallback(sym, start, end)

    rows   = [{"date":r[0],"open":r[1] or 0,"high":r[2] or 0,"low":r[3] or 0,"close":r[4],"volume":r[5] or 0} for r in rows_db]
...
def _live_fallback(sym, start, end):
    ...
    df = yf_fetch(lambda: yf.Ticker(f"{sym}.NS").history(start=start, end=end, interval=interval, auto_adjust=True))
    if df is None or df.empty:
        return {"ok":False, "error":f"No data available for {sym} in the selected range."}
    info = {}
    try:
    ...
    for idx, r in df.iterrows():
        ...
        rows.append({"date":str(idx.date()),
                     ...
                     "low": round(float(r.get("Low", 0) or 0),2),"close":round(c,2),"volume":int(r.get("Volume",0) or 0)})
    if rows:
        conn = get_db()
        with DB_LOCK:
            conn.executemany("INSERT OR REPLACE INTO prices(symbol,date,open,high,low,close,volume) VALUES(?,?,?,?,?,?,?)",
                [(sym,r["date"],r["open"],r["high"],r["low"],r["close"],r["volume"]) for r in rows])
            today = str(datetime.date.today())
            conn.execute("""INSERT INTO history_meta(symbol,oldest_date,newest_date,last_price_sync)
                VALUES(?,?,?,?) ON CONFLICT(symbol) DO UPDATE SET newest_date=excluded.newest_date,last_price_sync=excluded.last_price_sync
...
def fetch_training_data(sym):
    ...
    df = yf_fetch(lambda: yf.Ticker(f"{sym}.NS").history(start=str(start), end=str(end), interval="1d", auto_adjust=True))
...
def compute_indicators(sym):
    ...
    fund = conn.execute(
        "SELECT pe,forward_pe,revenue_growth,roe,debt_equity,insider_pct,inst_pct,sector,industry,high52,low52 FROM fundamentals WHERE symbol=?", (sym,)
    ).fetchone()
    conn.close()

    if not rows_db:
        # no DB data -> full live compute
        df = yf_fetch(lambda: yf.Ticker(f"{sym}.NS").history(start=start, end=str(datetime.date.today()+datetime.timedelta(days=1)), interval="1d", auto_adjust=True))
        if df is None or df.empty:
            raise ValueError(f"No data for {sym}.NS")
        info = yf_fetch(lambda: yf.Ticker(f"{sym}.NS").info or {}, retries=3, base_delay=4) or {}
        fin  = yf_fetch(lambda: yf.Ticker(f"{sym}.NS").financials, retries=3, base_delay=4)
        return _build_indicators_from_live(sym, df.dropna(subset=["Close"]), info, fin)
    ...
    for key, val, n in [("ma50",ma50,50),("ma200",ma200,200)]:
        if val is None:
            result[key] = {"value":"N/A","signal":"N/A","note":f"Need {n}+ days"}
        else:
            pct = round((last-val)/val*100, 2)
            sig = "BUY" if last>val and pct>2 else ("AVOID" if last<val else "CAUTION")
            result[key] = {"value":f"Rs{val:.2f}","pct_above":pct,"signal":sig,
    ...
    result["rsi"] = {"value":round(rsi_val,1),
        ...
        "note":"Oversold" if rsi_val<30 else ("Overbought" if rsi_val>70 else "Neutral (30-70)")}

    if fund:
        pe_raw,fpe_raw,rev_g,roe_r,de_r,ins,inst,sec,ind,h52,l52 = fund
    else:
        pe_raw=fpe_raw=rev_g=roe_r=de_r=ins=inst=None; sec=ind="N/A"; h52=l52=None

    pe_use = pe_raw or fpe_raw
    if pe_use is None:
...
def _build_indicators_from_live(sym, df, info, fin):
    ...
    for key,val,n in [("ma50",ma50,50),("ma200",ma200,200)]:
        if val is None: result[key]={"value":"N/A","signal":"N/A","note":f"Need {n}+ days"}
            ...
            result[key]={"value":f"Rs{val:.2f}","pct_above":pct,"signal":sig,"note":f"Price {'above' if last>val else 'below'} MA{n} by {abs(pct):.1f}%"}
    ...
    for key,sk,mult,t0,t1 in [("roe","returnOnEquity",100,15,8),("debt_equity","debtToEquity",0.01,0.5,1.0)]:
        ...
        if raw is None: result[key]={"value":"N/A","signal":"N/A","note":f"{key} not available"}
            ...
            result[key]={"value":f"{v}%" if mult==100 else f"{v}x","signal":sg,"note":f"{key.replace('_',' ')} = {v}"}
    ...
    h52=safe("fiftyTwoWeekHigh") or float(closes.tail(252).max()); l52=safe("fiftyTwoWeekLow") or float(closes.tail(252).min())
...
# ============================================================
# LSTM + PROPHET (logic unchanged, uses DB-backed fetch_training_data)
# ============================================================
SEQ_LEN   = 30
...
def forecast_lstm(sym,days):
    ...
    for ep in range(80):
        ...
        if vl<best_val-1e-5: best_val,best_state,pat=vl,{k:v.clone() for k,v in model.state_dict().items()},0
            ...
            pat+=1
            if pat>=12: print(f"  [LSTM] early stop ep {ep+1}"); break
...
def forecast_ensemble(sym,days):
    ...
    return {"model":"Ensemble (LSTM + Prophet)",
            ...
            "last_actual_date":results[0]["last_actual_date"],"last_actual_price":results[0]["last_actual_price"],"forecast":bl}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path=="/ping":
            ...
            self.send_json({"ok":True,"lstm":TORCH_OK,"prophet":PROPHET_OK,"db_mb":db_mb}); return
        if parsed.path=="/db_status":
            conn=get_db()
            rows=conn.execute("SELECT symbol,oldest_date,newest_date,last_price_sync,last_fund_sync FROM history_meta ORDER BY symbol").fetchall()
            cnt=conn.execute("SELECT COUNT(*) FROM prices").fetchone()[0]; conn.close()
            self.send_json({"ok":True,"total_rows":cnt,"symbols":[{"sym":r[0],"oldest":r[1],"newest":r[2],"last_price":r[3],"last_fund":r[4]} for r in rows]}); return
        if parsed.path=="/history":
            sym=qs.get("sym",[""])[0].upper().strip(); start=qs.get("start",[""])[0].strip(); end=qs.get("end",[""])[0].strip()
            if not sym or not start or not end: self.send_json({"ok":False,"error":"Missing sym/start/end"},400); return
            print(f"  History {sym}.NS {start}->{end}")
            try: self.send_json(get_stock_data(sym,start,end))
            except: self.send_json({"ok":False,"error":traceback.format_exc()},500)
            return
        if parsed.path=="/indicators":
            sym=qs.get("sym",[""])[0].upper().strip()
            if not sym: self.send_json({"ok":False,"error":"Missing sym"},400); return
            print(f"  Indicators {sym}.NS")
            try: data=compute_indicators(sym); data["ok"]=True; self.send_json(data)
            except: self.send_json({"ok":False,"error":traceback.format_exc()},500)
            return
        if parsed.path=="/forecast":
            sym=qs.get("sym",[""])[0].upper().strip(); days=max(7,min(30,int(qs.get("days",["14"])[0]))); model=qs.get("model",["ensemble"])[0].lower()
            if not sym: self.send_json({"ok":False,"error":"Missing sym"},400); return
            print(f"  Forecast {sym}.NS days={days} model={model}")
            try:
                r=(forecast_lstm(sym,days) if model=="lstm" else forecast_prophet(sym,days) if model=="prophet" else forecast_ensemble(sym,days))
                r["ok"]=True; self.send_json(r)
            except: self.send_json({"ok":False,"error":traceback.format_exc()},500)
            return
...
if __name__=="__main__":
    ...
    print("="*62)
    print("  NSE Stock Analyzer -- Server v6  (SQLite-backed)")
    print(f"  http://localhost:{PORT}")
    print(f"  Database : {DB_PATH}  ({db_mb} MB)")
    print(f"  LSTM     : {'OK' if TORCH_OK else 'MISSING -- pip install torch scikit-learn'}")
    print(f"  Prophet  : {'OK' if PROPHET_OK else 'MISSING -- pip install prophet'}")
    print(f"  Schedule : prices 5min | volume 1hr | fundamentals daily 08:00 IST")
    print("="*62)
