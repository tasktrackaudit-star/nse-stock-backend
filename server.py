...
def ensure_history(sym, conn=None):
    ...
    if row and row[1] == today:
        ...
        return
    start = row[0] if row else str(datetime.date.today() - datetime.timedelta(days=365*10+5))
    end   = str(datetime.date.today() + datetime.timedelta(days=1))
    print(f"  [DB] Seeding history for {sym} from {start}...")
    df = yf_fetch(lambda: yf.Ticker(f"{sym}.NS").history(start=start, end=end, interval="1d", auto_adjust=True))
    if df is None or df.empty:
        print(f"  [DB] Could not seed {sym} -- will use live fallback")
        if close_conn: conn.close()
        return
    rows = []
    ...
    with DB_LOCK:
        ...
        oldest = rows[0][1]  if rows else today
        newest = rows[-1][1] if rows else today
        conn.execute("""
            INSERT OR REPLACE INTO history_meta(symbol,oldest_date,newest_date,last_price_sync)
            VALUES(?,?,?,?)
        """, (sym, oldest, newest, today))
        conn.commit()
    print(f"  [DB] {sym}: {len(rows)} rows ({oldest} to {newest})")
    if close_conn: conn.close()
